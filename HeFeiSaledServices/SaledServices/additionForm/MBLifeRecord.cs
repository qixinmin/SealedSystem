...
namespace SaledServices
{
    public partial class MBLifeRecord : Form
    {
        ...
        {
            if (this.textBox8s.Text.Trim() == "")
            {
                MessageBox.Show("8s为空！");
                return;
            }
            try
            {
                ...
                while (querySdr.Read())
                {
                    tracknolist.Add( querySdr[0].ToString());
                }
                querySdr.Close();
                ///

                foreach(string str in tracknolist){
                    cmd.CommandText = "select Id, trackno, station,recordstatus,recorddate,repairplace1,repairnum1,repairMaterial1,inputer from  " + tableName + " where trackno='" + str.Trim() + "'";
                    querySdr = cmd.ExecuteReader();
                    while (querySdr.Read())
                    {
                        stationInfoRecord temp = new stationInfoRecord();
